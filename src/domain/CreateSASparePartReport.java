		...
		try {
			if (userSelection == JFileChooser.APPROVE_OPTION) {
				...
				if(data.size() > 0){
					...
					reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 2, false, true, false, false, true, false, 3000);
					row.createCell(3).setCellValue("Order Quantity");
					reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 3, false, true, false, false, false, true, 5000);
					row.createCell(4).setCellValue("SA Quantity");
					reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 4, false, true, false, false, false, true, 5000);
					row.createCell(5).setCellValue("SA Remain");
					reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 5, false, true, false, false, false, true, 5000);
					...
					while(cntData < data.size()){
						...
						reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 2, false, false, false, false, true, false, 3000);
						row.createCell(3).setCellValue(nf.format(data.get(cntData).getQuantity()));
						reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 3, false, false, false, false, false, true, 5000);
						row.createCell(4).setCellValue(nf.format(data.get(cntData).getSaUnit()));
						reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 4, false, false, false, false, false, true, 5000);
						row.createCell(5).setCellValue(nf.format(data.get(cntData).getSAremain()));
						reportDateVendorRowWithBorderCellStyle(workbook, row, sheet, 5, false, false, false, false, false, true, 5000);
